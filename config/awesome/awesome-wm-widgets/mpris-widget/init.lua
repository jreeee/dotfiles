...
local function worker(user_args)
    ...
    local font = args.font or 'FiraCode Nerd Font 16px'
    ...
    local update_corrected_metadata = function(track, artist, album, scobbles)
        metadata.current_song = track
        metadata.artist = artist
        metadata.album = album
        --local s = track .. '\nby ' .. artist .. album .. scobbles
        --print(s)
        metadata_widget:set_text(s)
    end

    -- THIS REQUIRES AWESOMEWM 4.4 (or the current git version!)
    naughty.connect_signal("request::display", function(n)
        -- get title and text
        if (n.app_name and n.app_name == "Firefox") then
            local correct_metadata = split_by_nl(n.message)
            local album = correct_metadata[3]
            local scrob = correct_metadata[4]
            if (scrob == nil) then
                scrob = album
                album = ''
            else
            ...
            --print(tprint(table.concat(correct_metadata))
            print('Track: ' .. correct_metadata[1])
            print('Artist:' .. correct_metadata[2])
            print('Album: ' .. album)
            print('Scrob:' .. scrob)
            
            update_corrected_metadata(correct_metadata[1], correct_metadata[2], album, scrob)
        end
        -- display as usual
    ...
    local update_metadata = function(meta)
        ...
        if art_url ~= nil and art_url ~= '' then
            ...
            -- hack bc yt thumbnails usally are 16:9 
            cover_art_widget.forced_height = popup_width/2
        else
